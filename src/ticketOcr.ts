...
function weightFor(text:string,label:'bruto'|'tara'|'neto'){
 for(const line of text.split(/\r?\n/)){
  const cleaned=line.trim();
  const labeled=new RegExp(`^(?:peso\\s*)?${label}\\b\\s*[:#-]?\\s*(${weight.source})`, 'i');
  const reverse=new RegExp(`^(${weight.source})\\s*(?:peso\\s*)?${label}\\b`, 'i');
  const match=cleaned.match(labeled)||cleaned.match(reverse);
  if(match){const value=match[1].match(weight);if(value)return number(value[1])+(value[2]?Number(`0.${value[2]}`):0)}
 }
 return undefined;
}
export function parseTicket(text:string,confidence:number):TicketReading{
 const fields:TicketFields={gross:weightFor(text,'bruto'),tare:weightFor(text,'tara'),printedNet:weightFor(text,'neto')};
 for(const line of text.split(/\r?\n/)){
  const folio=line.match(/\b(?:ID|folio|ticket|boleta|no\.?\s*de\s*ticket)\b\s*(?:n[úu]m(?:ero)?\.?|no\.?)?\s*[:#-]*\s*([A-Z0-9][A-Z0-9-]{1,39})/i);
