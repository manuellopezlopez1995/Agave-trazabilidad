...
export default function RealApp(){
 ...
 const configureTrip=(tripId:string)=>action(async()=>{if(!supabase||!isOnline)throw Error('Configurar carga requiere conexión');const destinationName=requiredText(destination,'Comprador y destino',120),plate=requiredText(vehiclePlate,'Placa',25);const {data,error}=await supabase.rpc('configure_harvest_trip',{p_trip_id:tripId,p_destination:destinationName,p_plate:plate});check(error);if(data!==tripId)throw Error('El servidor no confirmó la carga');setDestination('');setVehiclePlate('')});
