...
type Pane='home'|'farms'|'harvests'|'trips'|'deliveries'|'team'|'control'|'plantations';
const green='#164B3A';
...
export default function RealApp(){
 ...
 const finishDelivery=(d:Delivery)=>action(async()=>{
  ...
  const fieldKg=recordedFieldWeight(tripLots.filter(l=>l.trip_id===trip.id)),origin=weighings.find(w=>w.trip_id===trip.id&&w.weighing_type==='ORIGIN');
  ...
  if(difference.exceeded&&!varianceNotes.some(note=>note.trip_id===trip.id&&note.reason.trim().length>=10))throw Error(`${difference.exceededStages.map(stage=>`${stage.label}: ${stage.difference} kg`).join('; ')} supera la tolerancia de ${difference.threshold.toFixed(2)} kg. Registra una explicación antes de cerrar.`);
 ...
 const tripReview=(t:Trip)=>{
  ...
  const comparison=variance(dest?field:null,dest?origin?.net_weight_kg??null:null,dest?.net_weight_kg??null,accepted,rejected,varianceLimitKg,varianceLimitPercent);
  ...
  {trips.filter(t=>profile?.role!=='DRIVER'||t.driver_id===session.user.id).map(t=><View key={t.id} style={styles.card}>
   ...
   {selected===t.id&&<>
    ...
    {(profile?.role==='ADMIN'||profile?.role==='DRIVER'&&t.driver_id===session.user.id)&&<>
     ...
     {t.status==='ARRIVED'&&deliveries.filter(d=>d.trip_id===t.id&&d.status==='PENDING').map(d=><View key={d.id}><Text style={styles.heading}>Recepción pendiente · {d.recipient_company}</Text><Text style={styles.muted}>Para cerrar: registra el nombre de quien recibe, kilos aceptados y rechazados, y fotografía legible del recibo. El cierre confirmará la entrega.</Text><Button label="Continuar a recepción" onPress={()=>{setSelected(d.id);setPane('deliveries')}} disabled={busy||!reviewReady||(tripReview(t).comparison.exceeded&&!tripReview(t).hasExplanation)}/></View>)}
    </>}
   </>}
  </View>)}
 ...
 {pane==='deliveries'&&<>
  <SectionTitle eyebrow="04 / DESTINO" title="Recepción de entregas" description="Registra la recepción final y el comprobante antes de marcar el viaje como entregado."/>
  {deliveries.map(d=><View key={d.id} style={styles.card}>
   ...
   {selected===d.id&&<>
    ...
    {(profile?.role==='ADMIN'||profile?.role==='DRIVER'&&trips.find(t=>t.id===d.trip_id)?.driver_id===session.user.id)&&d.status!=='COMPLETED'&&<>
     <Button label="Fotografiar recibo" onPress={()=>addPhoto('delivery',d.id)} disabled={busy}/><Input label="Nombre de quien recibe" value={receiver} onChange={setReceiver}/><Input label="Peso aceptado (kg)" value={accepted} onChange={setAccepted}/><Input label="Peso rechazado (kg)" value={rejected} onChange={setRejected}/><Input label="Motivo del rechazo (si aplica)" value={rejectionReason} onChange={setRejectionReason}/><Button label="Cerrar recepción / Finalizar entrega" onPress={()=>confirm('Cerrar entrega','Los pesos, el receptor y el recibo quedarán registrados y el viaje se marcará como entregado.',()=>void finishDelivery(d))} disabled={busy||!reviewReady||trips.find(t=>t.id===d.trip_id)?.status!=='ARRIVED'||(trips.some(t=>t.id===d.trip_id&&tripReview(t).comparison.exceeded&&!tripReview(t).hasExplanation))}/>
    </>}
   </>}
  </View>)}
 </>}
