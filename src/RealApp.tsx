...
export default function RealApp(){
 ...
 const finishDelivery=(d:Delivery)=>action(async()=>{
  ...
  if(difference.exceeded&&!varianceNotes.some(note=>note.trip_id===trip.id&&note.reason.trim()))throw Error(`${difference.exceededStages.map(stage=>`${stage.label}: ${stage.difference} kg`).join('; ')} supera la tolerancia de ${difference.threshold.toFixed(2)} kg. Registra una explicación antes de cerrar.`);
