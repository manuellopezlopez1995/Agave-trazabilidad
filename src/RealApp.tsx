...
import {pendingTripArrivals,queueTripArrival,syncTripArrivals} from './offlineTripEvents';
import {partitionByBuyer,variance,recordedFieldWeight,DEFAULT_VARIANCE_KG,DEFAULT_VARIANCE_PERCENT} from './operations';

type Role='ADMIN'|'CREW_LEADER'|'DRIVER';
type Profile={id:string;full_name:string;role:Role;status:string};
type Farm={id:string;code:string;name:string;municipality:string|null;state:string;area_hectares:number|null;plantation_id:string|null};
...
type ScheduleRow={id:string;farmId:string;crewId:string;date:string};
...
type Trip={id:string;trace_code:string;plantation_folio:string|null;plantation_trip_number:number|null;origin_farm_id:string|null;harvest_order_id:string|null;driver_id:string|null;assigned_at:string|null;at_field_at:string|null;status:string;destination_name:string|null;departed_at:string|null;arrived_at:string|null;delivered_at?:string|null;vehicle_plate?:string|null};
type AvailableTrip={trip_id:string;trip_code:string;folio:string|null;harvest_id:string;harvest_code:string;farm:string;plantation_id:string|null;crew:string|null;date:string};
type TripEvent={id:string;trip_id:string;actor_id:string|null;old_status:string|null;new_status:string;event_type:string;captured_at:string|null;recorded_at:string;latitude:number|null;longitude:number|null};
...
type Closure={trip_id:string;approved_at:string;approved_by:string};
type FinalDossier={id:string;harvest_order_id:string;buyer_name:string;version:number;cutoff_at:string;issued_by:string;sha256:string;snapshot:any};
type Pane='home'|'farms'|'harvests'|'trips'|'team'|'control'|'plantations';
const green='#164B3A';
...
export default function RealApp(){
 ...
 const [pane,setPane]=useState<Pane>('home'),[selected,setSelected]=useState(''),[plantationSearch,setPlantationSearch]=useState(''),[farmName,setFarmName]=useState(''),[farmState,setFarmState]=useState('Jalisco'),[newPlantationId,setNewPlantationId]=useState(''),[assignFarmId,setAssignFarmId]=useState(''),[assignPlantationId,setAssignPlantationId]=useState('');
 const [scheduleRows,setScheduleRows]=useState<ScheduleRow[]>([{id:'first',farmId:'',crewId:'',date:jaliscoDate()}]);
 ...
 const load=useCallback(async()=>{if(!supabase||!session)return;setError('');try{
  ...
  const [f,c,h,l,t,tl,d,w,hp,dp,dr]=await Promise.all([
   supabase.from('farms').select('id,code,name,municipality,state,area_hectares,plantation_id').eq('organization_id',org).order('created_at',{ascending:false}),
   ...
   supabase.from('trips').select('id,trace_code,plantation_folio,plantation_trip_number,origin_farm_id,harvest_order_id,driver_id,assigned_at,at_field_at,status,destination_name,departed_at,arrived_at,delivered_at,vehicle_plate').eq('organization_id',org).order('created_at',{ascending:false}),
  ...
  const [eventResult,availableResult]=await Promise.all([
   supabase.from('trip_status_events').select('id,trip_id,actor_id,old_status,new_status,event_type,captured_at,recorded_at,latitude,longitude').eq('organization_id',org).order('recorded_at',{ascending:true}),
   p.role==='DRIVER'?supabase.rpc('driver_available_trips'):Promise.resolve({data:[],error:null})
  ]);check(eventResult.error);check(availableResult.error);setTripEvents((eventResult.data??[]) as TripEvent[]);setAvailableTrips((availableResult.data??[]) as AvailableTrip[]);
  const [settings,reviews,correctionResult,closureResult,dossierResult]=await Promise.all([supabase.from('organization_variance_settings').select('limit_kg,limit_percent').eq('organization_id',org).maybeSingle(),supabase.from('trip_variance_reviews').select('trip_id,reason,created_at').eq('organization_id',org).order('created_at',{ascending:false}),supabase.from('trip_correction_notes').select('trip_id,field_name,corrected_value,reason,created_at,created_by').eq('organization_id',org).order('created_at',{ascending:false}),supabase.from('trip_closures').select('trip_id,approved_at,approved_by').eq('organization_id',org),p.role==='ADMIN'?supabase.from('buyer_dossier_versions').select('id,harvest_order_id,buyer_name,version,cutoff_at,issued_by,sha256,snapshot').eq('organization_id',org).order('cutoff_at',{ascending:false}):Promise.resolve({data:[],error:null})]);
  ...
  setFormalReady(!closureResult.error&&!dossierResult.error);setClosures((closureResult.data??[]) as Closure[]);setFinalDossiers((dossierResult.data??[]) as FinalDossier[]);
 ...
 const syncPending=()=>action(async()=>{if(!session||!organizationId||Platform.OS!=='web')throw Error('Abre tu sesión con conexión');await syncEvidence(organizationId,session.user.id,setPendingCount);setPendingCount((await pendingEvidence()).length)});
 const syncPendingTrips=()=>action(async()=>{if(!session||!isOnline)throw Error('Se necesita conexión para confirmar los eventos');setPendingTripCount(await syncTripArrivals(session.user.id))});
 const claimTrip=(tripId:string)=>action(async()=>{if(!supabase||!isOnline)throw Error('Tomar viaje requiere confirmación del servidor y conexión');const {data,error}=await supabase.rpc('claim_harvest_trip',{p_trip_id:tripId});check(error);if(data!=='ASSIGNED')throw Error('El servidor no confirmó la asignación');setSelected(tripId)});
 const assignTrip=(tripId:string,driverId:string)=>action(async()=>{if(!supabase||!isOnline)throw Error('Asignar chofer requiere conexión');const {data,error}=await supabase.rpc('assign_harvest_trip_driver',{p_trip_id:tripId,p_driver_id:driverId});check(error);if(data!=='ASSIGNED')throw Error('El servidor no confirmó la asignación')});
 const configureTrip=(trip:Trip)=>action(async()=>{if(!supabase||!isOnline)throw Error('Configurar carga requiere conexión');const destinationName=requiredText(destination||trip.destination_name||'','Comprador y destino',120),plate=requiredText(vehiclePlate||trip.vehicle_plate||'','Placa',25);const {data,error}=await supabase.rpc('configure_harvest_trip',{p_trip_id:trip.id,p_destination:destinationName,p_plate:plate});check(error);if(data!==trip.id)throw Error('El servidor no confirmó la carga');setDestination('');setVehiclePlate('')});
 const arriveAtField=(trip:Trip)=>action(async()=>{
  if(!supabase||!session)throw Error('Abre tu sesión de chofer');
  const event={tripId:trip.id,eventId:crypto.randomUUID(),actorId:session.user.id,capturedAt:new Date().toISOString(),latitude:null as number|null,longitude:null as number|null};
  if(Platform.OS==='web'&&isOnline&&navigator.geolocation&&navigator.permissions){
   try{const permission=await navigator.permissions.query({name:'geolocation'});if(permission.state==='granted'){
    const point=await new Promise<GeolocationPosition>((resolve,reject)=>navigator.geolocation.getCurrentPosition(resolve,reject,{enableHighAccuracy:true,timeout:5000,maximumAge:30000}));
    event.latitude=point.coords.latitude;event.longitude=point.coords.longitude;
   }}catch{/* GPS is optional; preserve the recorded arrival. */}
  }
  if(Platform.OS==='web'&&!isOnline){queueTripArrival(event);setPendingTripCount(pendingTripArrivals().length);showError('Llegada guardada en este dispositivo. Se confirmará al recuperar conexión; no borres los datos locales.');return}
  const {data,error}=await supabase.rpc('mark_trip_at_field',{p_trip_id:event.tripId,p_event_id:event.eventId,p_captured_at:event.capturedAt,p_latitude:event.latitude,p_longitude:event.longitude});
  if(error){if(Platform.OS==='web'&&!navigator.onLine){queueTripArrival(event);setPendingTripCount(pendingTripArrivals().length);return}check(error)}
 ...
 const validPlantationId=(value:string)=>{const id=value.trim();if(!/^[0-9]{1,20}$/.test(id))throw Error('El ID de plantación debe contener de 1 a 20 dígitos');return id};
 const createFarm=()=>action(async()=>{if(!supabase||!session||!organizationId)throw Error('Sesión inválida');const name=requiredText(farmName,'Nombre',120),state=requiredText(farmState,'Estado',80),plantation_id=validPlantationId(newPlantationId);const {error}=await supabase.from('farms').insert({organization_id:organizationId,name,state,plantation_id,country:'México',created_by:session.user.id});check(error);setFarmName('');setNewPlantationId('')});
 const assignPlantation=()=>action(async()=>{if(!supabase||!assignFarmId)throw Error('Selecciona un predio');const {error}=await supabase.rpc('assign_plantation_id',{p_farm_id:assignFarmId,p_plantation_id:validPlantationId(assignPlantationId)});check(error);setAssignFarmId('');setAssignPlantationId('')});
 const setScheduleRow=(id:string,patch:Partial<ScheduleRow>)=>setScheduleRows(rows=>rows.map(row=>row.id===id?{...row,...patch}:row));
 const createHarvest=()=>action(async()=>{
  if(!supabase||!session||!organizationId||profile?.role!=='ADMIN')throw Error('Sesión de administrador requerida');
  const seen=new Set<string>();
  const items=scheduleRows.map((row,index)=>{
   if(!row.farmId||!farms.some(f=>f.id===row.farmId))throw Error(`Jima ${index+1}: selecciona un predio`);
   if(!row.crewId||!crews.some(c=>c.id===row.crewId))throw Error(`Jima ${index+1}: selecciona una cuadrilla`);
   if(!row.date.trim())throw Error(`Jima ${index+1}: selecciona una fecha`);
  ...
  const {data,error}=await supabase.rpc('schedule_harvest_batch',{p_organization_id:organizationId,p_items:items});check(error);
  if(!Array.isArray(data)||data.length!==items.length)throw Error('El servidor no confirmó todas las jimas');
  setScheduleRows([{id:`new-${Date.now()}`,farmId:'',crewId:'',date:jaliscoDate()}]);setSchedulePicker('');
 });
 const harvestMissing=(h:Harvest)=>{const ownLots=lots.filter(l=>l.harvest_order_id===h.id);return [!ownLots.length?'Registra al menos un lote':null,...ownLots.filter(l=>!l.agave_count||l.average_brix==null||l.actual_weight_kg==null||l.actual_weight_kg<=0).map(l=>`Completa las mediciones de ${l.trace_code}`),!harvestPhotos.some(p=>p.harvest_order_id===h.id)?'Sincroniza o toma una fotografía de la jima':null].filter((v):v is string=>!!v)};
 const updateHarvest=(h:Harvest)=>action(async()=>{if(!supabase)throw Error('Falta configurar Supabase');const next=h.status==='ASSIGNED'?'IN_PROGRESS':h.status==='IN_PROGRESS'?'HARVESTED':null;if(!next)throw Error('Esta jima no tiene avance disponible');if(next==='HARVESTED'){const missing=harvestMissing(h);if(missing.length)throw Error(`Antes de terminar la jima: ${missing.join('; ')}`)}const {data,error}=await supabase.rpc('advance_harvest',{p_harvest_id:h.id});check(error);if(data!==next)throw Error('El servidor no confirmó el nuevo estado de la jima')});
 const addLot=(h:Harvest)=>action(async()=>{
  ...
  if(editingLotId)throw Error('Termina o cancela la edición del lote antes de registrar otro');const {error}=await supabase.from('agave_lots').insert({organization_id:organizationId,harvest_order_id:h.id,farm_id:h.farm_id,agave_count:count,average_brix:brix,actual_weight_kg:weight,harvest_date:jaliscoDate(),created_by:session.user.id});check(error);setLotCount('');setLotBrix('');setLotWeight('');
 });
 const measureLot=(lot:Lot)=>action(async()=>{
  if(!supabase)throw Error('Falta configurar Supabase');const harvest=harvests.find(h=>h.id===lot.harvest_order_id);if(editingLotId!==lot.id)throw Error('Selecciona primero este lote para editarlo');if(harvest?.status!=='IN_PROGRESS'||lot.status!=='OPEN')throw Error('Sólo se puede medir un lote abierto durante una jima en curso');const count=integer(lotCount,'Agaves cosechados',1,100000),brix=decimal(lotBrix,'Brix promedio',0,50),weight=decimal(lotWeight,'Peso del lote',0.01,200000);
  const {data,error}=await supabase.from('agave_lots').update({agave_count:count,average_brix:brix,actual_weight_kg:weight}).eq('id',lot.id).eq('status','OPEN').select('id');check(error);if(!data?.length)throw Error('No se actualizó el lote; revisa permisos RLS');setEditingLotId('');setLotCount('');setLotBrix('');setLotWeight('');
 });
 const addPhoto=(kind:'harvest'|'delivery',id:string)=>action(async()=>{
  ...
  if(photo.queued){setPendingCount((await pendingEvidence()).length);const notice='Fotografía guardada sólo en este dispositivo. Sincronízala antes de terminar la jima o borrar los datos del navegador.';if(Platform.OS==='web')window.alert(notice);else Alert.alert('Fotografía pendiente',notice);return}
  if(kind==='harvest'){
 ...
 const downloadTripPdf=async(t:Trip)=>{
  setExporting(true);setError('');try{
   ...
   const harvestIds=[...new Set([...tripLotsData.map(l=>l.harvest_order_id),...(t.harvest_order_id?[t.harvest_order_id]:[])])];
   const [hr,fr,dr,hp,ws]=await Promise.all([
    ...
    client.from('weighings').select('weighing_type,gross_weight_kg,tare_weight_kg,net_weight_kg,ticket_number,ticket_storage_path,storage_bucket,captured_at,recorded_by,latitude,longitude').eq('trip_id',t.id)
 ...
 const createTrip=()=>action(async()=>{
  ...
  const plate=requiredText(vehiclePlate,'Placa del camión',25).toUpperCase();if(!farms.find(f=>f.id===lot.farm_id)?.plantation_id)throw Error('Primero asigna el ID de plantación al predio');
 ...
 const startTicket=async(trip:Trip)=>{
  ...
  try{const photo=await photographed;if(!photo)return;
   ...
   try{reading=await readTicket(photo.blob)}catch(e){console.error('Error al analizar el ticket de báscula',e);reading={fields:{},confidence:0,fieldConfidence:{},warnings:['No pudimos leer automáticamente el ticket. Intenta tomar otra fotografía o captura los datos manualmente.']}}
 ...
 const addWeighing=(trip:Trip)=>action(async()=>{
  ...
  const ocrReading={engine:'tesseract.js',confidence:ticketDraft.reading.confidence,fieldConfidence:ticketDraft.reading.fieldConfidence,detected:{gross:original.gross??null,tare:original.tare??null,printedNet:original.printedNet??null,calculatedNet:ticketDraft.reading.netKg??null,folio:original.folio??null,date:original.date??null,time:original.time??null}};
 ...
 const approveTrip=(trip:Trip)=>action(async()=>{if(!supabase||!formalReady||profile?.role!=='ADMIN')throw Error('Activa primero el cierre formal en Supabase');const {error}=await supabase.rpc('approve_trip_closure',{p_trip_id:trip.id});check(error)});
 const issueDossier=(h:Harvest,buyer:string)=>action(async()=>{if(!supabase||!formalReady||profile?.role!=='ADMIN')throw Error('Activa primero el expediente final en Supabase');const {error}=await supabase.rpc('finalize_buyer_dossier',{p_harvest_id:h.id,p_buyer:buyer});check(error)});
 const downloadIssued=async(issued:FinalDossier)=>{
  if(!supabase||Platform.OS!=='web'||exporting)return;setExporting(true);
  try{
   const {data:verified,error:verificationError}=await supabase.rpc('verify_buyer_dossier',{p_id:issued.id});check(verificationError);if(!verified)throw Error('La huella de esta versión no coincide con la instantánea guardada');
 ...
 const advanceTrip=(trip:Trip)=>action(async()=>{
  ...
  if(trip.status==='AT_FIELD')transitions.AT_FIELD='LOADING';if(trip.status==='ASSIGNED'&&trip.harvest_order_id)throw Error('Primero registra la llegada al predio');
 ...
 const finishDelivery=(trip:Trip)=>action(async()=>{
  if(!supabase||!isOnline)throw Error('Finalizar entrega requiere conexión y confirmación del servidor');
  if(trip.status!=='ARRIVED'||!deliveries.some(d=>d.trip_id===trip.id&&d.status==='PENDING'))throw Error('Se necesita un viaje llegado con una entrega pendiente');
  if(!reviewReady)throw Error('No se pudo comprobar la configuración de conciliación');
  if(!['ORIGIN','DESTINATION'].every(kind=>weighings.some(w=>w.trip_id===trip.id&&w.weighing_type===kind&&w.ticket_storage_path&&w.ticket_number?.trim()&&w.legibility_confirmed)))throw Error('Se requieren los dos tickets con folio y fotografía legible sincronizada');
  const review=tripReview(trip);
  if(review.comparison.exceeded&&!review.hasExplanation)throw Error('Registra una explicación válida antes de finalizar la entrega');
  const {data,error}=await supabase.rpc('complete_delivery_from_destination',{p_trip_id:trip.id});check(error);if(data!=='DELIVERED')throw Error('El servidor no confirmó el cierre de la entrega');
 });
 const tripReview=(t:Trip)=>{
  const field=recordedFieldWeight(tripLots.filter(l=>l.trip_id===t.id));
  ...
  // La conciliación se presenta al contar con el pesaje de destino; no se alerta durante la carga.
  const comparison=variance(dest?field:null,dest?origin?.net_weight_kg??null:null,dest?.net_weight_kg??null,null,null,varianceLimitKg,varianceLimitPercent);
  const missing=[!origin?'pesaje origen':null,!dest?'pesaje destino':null,...ws.filter(w=>!w.ticket_storage_path||!w.ticket_number?.trim()||!w.legibility_confirmed).map(w=>`ticket ${w.weighing_type}`),!ds.length?'entrega':null,...ds.filter(d=>d.status!=='COMPLETED').map(d=>`entrega ${d.trace_code} pendiente`)].filter(Boolean);
  return {comparison,missing,hasExplanation:varianceNotes.some(n=>n.trip_id===t.id&&n.reason?.trim().length>=10)};
 };
 const departureMissing=(t:Trip)=>t.status!=='LOADING'?[]:[
  !tripLots.some(l=>l.trip_id===t.id)?'lote cosechado':null,
  tripLots.some(link=>link.trip_id===t.id&&lots.find(l=>l.id===link.agave_lot_id)?.status!=='HARVESTED')?'cosechar lotes':null,
  !weighings.some(w=>w.trip_id===t.id&&w.weighing_type==='ORIGIN')?'pesaje de origen':null,
  t.harvest_order_id&&!t.driver_id?'chofer':null,
  t.harvest_order_id&&!t.vehicle_plate?.trim()?'placa':null,
  t.harvest_order_id&&!t.destination_name?.trim()?'destino':null,
  t.harvest_order_id&&!deliveries.some(d=>d.trip_id===t.id&&d.status==='PENDING'&&d.recipient_company?.trim().toLocaleLowerCase('es-MX')===t.destination_name?.trim().toLocaleLowerCase('es-MX'))?'entrega pendiente':null
 ...
 const plantationFolders=farms.filter(f=>!!f.plantation_id&&trips.some(t=>t.origin_farm_id===f.id));
 const matchingPlantationFolders=plantationFolders.filter(f=>f.plantation_id!.includes(plantationSearch.trim()));
 const activeHarvests=harvests.filter(h=>h.status!=='HARVESTED'&&h.status!=='CANCELLED');
 const harvestedHarvests=harvests.filter(h=>h.status==='HARVESTED');
 const harvestCard=(h:Harvest)=><View key={h.id} style={styles.card}>
   <Pressable onPress={()=>{setSelected(selected===h.id?'':h.id);setEditingLotId('');setLotCount('');setLotBrix('');setLotWeight('')}}><Text style={styles.heading}>{h.trace_code} {selected===h.id?'▲':'▼'}</Text><Text>{farms.find(f=>f.id===h.farm_id)?.name??'Predio'} · {h.status} · {h.scheduled_date??'Sin fecha'}</Text></Pressable>
   {selected===h.id&&<>
    <Text>ID de plantación: {farms.find(f=>f.id===h.farm_id)?.plantation_id??'Sin asignar'}</Text><Text>Cuadrilla: {crews.find(c=>c.id===h.crew_id)?.name??'Sin asignar'}</Text><Text>Fecha programada: {h.scheduled_date??'Sin fecha'} · Finalización: {h.completed_at?new Date(h.completed_at).toLocaleString('es-MX'):'Pendiente'}</Text><Text>Estado: {h.status}</Text>
    {h.notes?<Text>{h.notes}</Text>:null}
    {trips.filter(t=>t.harvest_order_id===h.id).map(t=><View key={t.id}><Text>Viaje ligado: {t.plantation_folio??t.trace_code} · {t.status} · {t.destination_name??'Destino pendiente'}</Text><Button label={`Abrir trazabilidad de ${t.plantation_folio??t.trace_code}`} onPress={()=>{setPane('trips');setSelected(t.id)}}/></View>)}
    {profile?.role==='ADMIN'&&(()=>{const result=buyerGroupsForHarvest(h);return <>{result.message?<Text style={styles.error}>{result.message}</Text>:null}{result.groups.map(group=><View key={group.key}><Button label={exporting?'Preparando expediente…':`Descargar borrador · ${group.name}`} onPress={()=>void exportHarvest(h,group.key)} disabled={exporting||busy}/>{formalReady&&group.trips.length>0&&group.trips.every(t=>closures.some(c=>c.trip_id===t.id))&&<Button label={`Emitir versión final · ${group.name}`} onPress={()=>confirm('Emitir expediente',`Se fijará una nueva versión del expediente para ${group.name}.`,()=>void issueDossier(h,group.name))} disabled={busy}/>}</View>)}{finalDossiers.filter(d=>d.harvest_order_id===h.id).map(d=><View key={d.id} style={styles.card}><Text>Versión final {d.version} · {d.buyer_name} · corte {new Date(d.cutoff_at).toLocaleString('es-MX')}</Text><Text style={styles.small}>SHA-256: {d.sha256}</Text><Button label="Descargar versión final" onPress={()=>void downloadIssued(d)} disabled={exporting}/>{corrections.filter(c=>new Date(c.created_at)>new Date(d.cutoff_at)&&d.snapshot.trip_ids?.includes(c.trip_id)).map((c,i)=><Text key={i} style={styles.error}>Ajuste posterior: {c.field_name} · {c.reason}</Text>)}</View>)}</>})()}
    {lots.filter(l=>l.harvest_order_id===h.id).map(l=><View key={l.id}><Text>Lote {l.trace_code}: {l.agave_count??'—'} agaves · {l.average_brix??'—'} °Brix · {l.actual_weight_kg??'—'} kg</Text>{h.status==='IN_PROGRESS'&&l.status==='OPEN'&&(profile?.role==='ADMIN'||profile?.role==='CREW_LEADER')&&<>{editingLotId===l.id?<><Text style={styles.muted}>Editando únicamente {l.trace_code}</Text><Input label="Agaves cosechados" value={lotCount} onChange={setLotCount}/><Input label="Brix promedio" value={lotBrix} onChange={setLotBrix}/><Input label="Peso del lote (kg)" value={lotWeight} onChange={setLotWeight}/><Button label={`Guardar cambios en ${l.trace_code}`} onPress={()=>measureLot(l)} disabled={busy}/><Button label="Cancelar edición" onPress={()=>{setEditingLotId('');setLotCount('');setLotBrix('');setLotWeight('')}} disabled={busy}/></>:<Button label={`Editar mediciones de ${l.trace_code}`} onPress={()=>{setEditingLotId(l.id);setLotCount(l.agave_count==null?'':String(l.agave_count));setLotBrix(l.average_brix==null?'':String(l.average_brix));setLotWeight(l.actual_weight_kg==null?'':String(l.actual_weight_kg))}} disabled={busy||!!editingLotId}/>}</>}</View>)}
    ...
    {(profile?.role==='CREW_LEADER'||profile?.role==='ADMIN')&&<>
     {h.status!=='HARVESTED'&&<Button label="Tomar foto de jima" onPress={()=>addPhoto('harvest',h.id)} disabled={busy}/>}
     {h.status==='IN_PROGRESS'&&!editingLotId&&<><Text style={styles.heading}>Registrar nuevo lote</Text><Input label="Agaves cosechados" value={lotCount} onChange={setLotCount}/><Input label="Brix promedio" value={lotBrix} onChange={setLotBrix}/><Input label="Peso del lote (kg)" value={lotWeight} onChange={setLotWeight}/><Button label="Registrar lote" onPress={()=>addLot(h)} disabled={busy}/></>}
     {h.status==='IN_PROGRESS'&&harvestMissing(h).length>0&&<Text style={styles.error}>Para terminar: {harvestMissing(h).join(' · ')}.</Text>}
     {(h.status==='ASSIGNED'||h.status==='IN_PROGRESS')&&<Button label={h.status==='ASSIGNED'?'Iniciar jima':'Terminar jima'} onPress={()=>confirm(h.status==='ASSIGNED'?'Iniciar jima':'Terminar jima',h.status==='ASSIGNED'?'La jima quedará en curso.':'La jima y sus lotes quedarán cerrados para operación.',()=>void updateHarvest(h))} disabled={busy||(h.status==='IN_PROGRESS'&&harvestMissing(h).length>0)}/>} 
    </>}
   </>}
  </View>;
 ...
 {pane==='farms'&&<><SectionTitle eyebrow="01 / ORIGEN" title="Predios visibles" description="Cada predio lleva un ID de plantación que no se puede cambiar después de asignarlo."/>{farms.map(f=><View key={f.id} style={styles.card}><Text style={styles.heading}>{f.name}</Text><Text>{f.code} · {f.municipality??f.state}</Text><Text>ID de plantación: {f.plantation_id??'Pendiente de asignar'}</Text>{!f.plantation_id&&profile?.role==='ADMIN'&&<><Button label="Asignar ID de plantación" onPress={()=>setAssignFarmId(f.id)}/>{assignFarmId===f.id&&<><Input label="ID de plantación (sólo dígitos)" value={assignPlantationId} onChange={setAssignPlantationId}/><Button label="Confirmar ID y foliar viajes anteriores" onPress={()=>confirm('Asignar ID definitivo',`El ID ${assignPlantationId} se usará para todos los viajes de este predio y no podrá cambiarse.`,()=>void assignPlantation())} disabled={busy}/></>}</>}</View>)}{profile?.role==='ADMIN'&&<><Text style={styles.heading}>Nuevo predio</Text><Text style={styles.muted}>Código interno: Se generará automáticamente al guardar.</Text><Input label="ID de plantación (sólo dígitos)" value={newPlantationId} onChange={setNewPlantationId}/><Input label="Nombre" value={farmName} onChange={setFarmName}/><Input label="Estado" value={farmState} onChange={setFarmState}/><Button label="Guardar predio" onPress={createFarm} disabled={busy}/></>}</>}
 {pane==='plantations'&&<><SectionTitle eyebrow="ARCHIVO / ORIGEN" title="Carpetas por plantación" description="Abre una carpeta y descarga el PDF de cada viaje. Los viajes pendientes se indican como borrador."/><View><Text style={styles.label}>Buscar carpeta por ID de plantación</Text><TextInput style={styles.input} value={plantationSearch} onChangeText={setPlantationSearch} placeholder="Ej. 123456" keyboardType="number-pad" accessibilityLabel="Buscar carpeta por ID de plantación"/></View>{plantationSearch.trim()?<Text style={styles.muted}>{matchingPlantationFolders.length} {matchingPlantationFolders.length===1?'carpeta encontrada':'carpetas encontradas'} · <Text onPress={()=>setPlantationSearch('')} style={{color:green,textDecorationLine:'underline'}}>Limpiar búsqueda</Text></Text>:null}{matchingPlantationFolders.map(f=><View key={f.id} style={styles.card}><Pressable onPress={()=>setSelected(selected===f.id?'':f.id)}><Text style={styles.heading}>📁 {f.plantation_id} {selected===f.id?'▲':'▼'}</Text><Text>{f.name} · {trips.filter(t=>t.origin_farm_id===f.id).length} viajes</Text></Pressable>{selected===f.id&&trips.filter(t=>t.origin_farm_id===f.id).sort((a,b)=>(a.plantation_trip_number??0)-(b.plantation_trip_number??0)).map(t=><View key={t.id} style={styles.folderFile}><Text style={styles.heading}>📄 {t.plantation_folio?`${t.plantation_folio}.pdf`:'Folio pendiente'}</Text><Text>{t.destination_name??'Sin comprador'} · {closures.some(c=>c.trip_id===t.id)?'Conciliado':'Borrador'}</Text>{t.plantation_folio?<><Button label={exporting?'Preparando PDF…':`Preparar ${t.plantation_folio}.pdf`} onPress={()=>void downloadTripPdf(t)} disabled={exporting||busy}/>{preparedPdf?.tripId===t.id&&Platform.OS==='web'&&<a href={preparedPdf.url} download={preparedPdf.name} target="_blank" rel="noopener noreferrer" style={{display:'block',padding:15,marginTop:8,backgroundColor:'#164B3A',color:'white',borderRadius:12,textAlign:'center',fontWeight:'bold'}}>Abrir o guardar {preparedPdf.name}</a>}</>:profile?.role==='ADMIN'?<Button label="Asignar folio pendiente" onPress={()=>void action(async()=>{if(!supabase||!f.plantation_id)throw Error('Falta ID de plantación');const {error}=await supabase.rpc('assign_plantation_id',{p_farm_id:f.id,p_plantation_id:f.plantation_id});check(error)})} disabled={busy}/>:null}</View>)}</View>)}{!plantationFolders.length?<Text style={styles.muted}>Aún no hay viajes con ID de plantación. Asigna el ID al predio para crear su carpeta.</Text>:plantationSearch.trim()&&!matchingPlantationFolders.length?<Text style={styles.muted}>No se encontraron carpetas para ese ID de plantación.</Text>:null}</>}
 {pane==='harvests'&&<>
  <SectionTitle eyebrow="02 / COSECHA" title="Jimas activas" description="Jimas programadas y en proceso. Las finalizadas pasan al historial Jimado."/>
  {activeHarvests.map(harvestCard)}
  {!activeHarvests.length&&<Text style={styles.muted}>No hay jimas activas.</Text>}
  {profile?.role==='ADMIN'&&<>
   <SectionTitle eyebrow="PLANIFICACIÓN / COSECHA" title="Programar jimas" description="Cada fila es una jima independiente. Todas se guardan juntas."/>
   {scheduleRows.map((row,index)=><View key={row.id} style={styles.card}>
    <Text style={styles.heading}>Jima {index+1}</Text>
    <View style={{flexDirection:'row',flexWrap:'wrap',gap:12}}>
     <View style={{flexBasis:190,flexGrow:1}}><Text style={styles.label}>Predio</Text><Button label={farms.find(f=>f.id===row.farmId)?.name??'Seleccionar predio ▾'} onPress={()=>setSchedulePicker(schedulePicker===`${row.id}:farm`?'':`${row.id}:farm`)}/>{schedulePicker===`${row.id}:farm`&&farms.map(f=><Button key={f.id} label={`${f.name} · ${f.plantation_id??'Sin ID'}`} onPress={()=>{setScheduleRow(row.id,{farmId:f.id});setSchedulePicker('')}}/>)}</View>
     <View style={{flexBasis:190,flexGrow:1}}><Text style={styles.label}>Cuadrilla</Text><Button label={crews.find(c=>c.id===row.crewId)?.name??'Seleccionar cuadrilla ▾'} onPress={()=>setSchedulePicker(schedulePicker===`${row.id}:crew`?'':`${row.id}:crew`)}/>{schedulePicker===`${row.id}:crew`&&crews.map(c=><Button key={c.id} label={c.name} onPress={()=>{setScheduleRow(row.id,{crewId:c.id});setSchedulePicker('')}}/>)}</View>
     <View style={{flexBasis:180,flexGrow:1}}><Input label="Fecha programada (AAAA-MM-DD)" value={row.date} onChange={date=>setScheduleRow(row.id,{date})}/></View>
    </View>
    {scheduleRows.length>1&&<Button label="Eliminar fila" onPress={()=>setScheduleRows(rows=>rows.filter(r=>r.id!==row.id))} disabled={busy}/>} 
   </View>)}
   <Button label="+ Agregar otra jima" onPress={()=>setScheduleRows(rows=>[...rows,{id:`row-${Date.now()}-${Math.random()}`,farmId:'',crewId:'',date:jaliscoDate()}])} disabled={busy}/>
   <Button label="PROGRAMAR JIMAS" onPress={createHarvest} disabled={busy}/>
  </>}
  <SectionTitle eyebrow="HISTORIAL / COSECHA" title="JIMADO" description="Jimas finalizadas con sus lotes, expedientes y fotografías originales."/>
  {harvestedHarvests.map(harvestCard)}
  {!harvestedHarvests.length&&<Text style={styles.muted}>Aún no hay jimas terminadas.</Text>}
 ...
 {pane==='trips'&&<>
  ...
  {trips.filter(t=>profile?.role!=='DRIVER'||t.driver_id===session.user.id).map(t=><View key={t.id} style={styles.card}>
   ...
   {selected===t.id&&<>
    {t.harvest_order_id&&<><Text>Jima: {harvests.find(h=>h.id===t.harvest_order_id)?.trace_code??t.harvest_order_id} · Predio: {farms.find(f=>f.id===t.origin_farm_id)?.name??'Vinculado en expediente'} · ID plantación: {farms.find(f=>f.id===t.origin_farm_id)?.plantation_id??'Pendiente'}</Text><Text>Cuadrilla: {crews.find(c=>c.id===harvests.find(h=>h.id===t.harvest_order_id)?.crew_id)?.name??'Consultar jima'} · Chofer: {team.find(p=>p.id===t.driver_id)?.full_name??drivers.find(p=>p.id===t.driver_id)?.full_name??'Pendiente'}</Text><Text>Asignación: {t.assigned_at?new Date(t.assigned_at).toLocaleString('es-MX'):'Pendiente'} · Llegada al predio: {t.at_field_at?new Date(t.at_field_at).toLocaleString('es-MX'):'Pendiente'}</Text></>}
    ...
    {profile?.role==='ADMIN'&&t.harvest_order_id&&['PENDING_DRIVER','ASSIGNED'].includes(t.status)&&<><Text style={styles.heading}>Asignación de chofer</Text>{drivers.map(d=><Button key={d.id} label={`${t.driver_id===d.id?'✓ ':''}${d.full_name}`} onPress={()=>confirm('Asignar chofer',`Asignar ${t.plantation_folio??t.trace_code} a ${d.full_name}`,()=>void assignTrip(t.id,d.id))} disabled={busy}/>)}</>}
    {t.harvest_order_id&&['PENDING_DRIVER','ASSIGNED','AT_FIELD','LOADING'].includes(t.status)&&(profile?.role==='ADMIN'||profile?.role==='DRIVER'&&t.driver_id===session.user.id)&&<><Text style={styles.heading}>Preparar salida</Text><Text style={styles.muted}>Destino actual: {t.destination_name??'Pendiente'} · Placa actual: {t.vehicle_plate??'Pendiente'}. La entrega se abrirá como pendiente y se cerrará en destino.</Text><Input label="Comprador / destilería de destino" value={destination} onChange={setDestination}/><Input label="Placa del camión" value={vehiclePlate} onChange={setVehiclePlate}/><Button label="Guardar destino, placa y entrega pendiente" onPress={()=>void configureTrip(t)} disabled={busy||!isOnline}/></>}
    ...
    {(profile?.role==='ADMIN'||profile?.role==='DRIVER'&&t.driver_id===session.user.id)&&<>
     ...
     {ticketDraft?.tripId===t.id&&<><Text style={styles.muted}>Lectura del ticket · revisa los valores antes de confirmar.</Text>{ticketDraft.reading.fields.folio&&ticketDraft.reading.fields.gross!==undefined&&ticketDraft.reading.fields.tare!==undefined&&ticketDraft.reading.netKg!==undefined&&ticketDraft.reading.netKg>0&&<Text style={styles.muted}>Folio: {ticketDraft.reading.fields.folio} · Bruto: {ticketDraft.reading.fields.gross.toLocaleString('es-MX')} kg · Tara: {ticketDraft.reading.fields.tare.toLocaleString('es-MX')} kg · Neto calculado: {ticketDraft.reading.netKg.toLocaleString('es-MX')} kg{ticketDraft.reading.fields.printedNet!==undefined&&Math.abs(ticketDraft.reading.netKg-ticketDraft.reading.fields.printedNet)<=2?' · coincide con el neto impreso':''}.</Text>}{ticketPreview&&<Image source={{uri:ticketPreview}} style={{width:'100%',height:250,resizeMode:'contain'}}/>}
     ...
     <Button label="Confirmar pesaje" onPress={()=>addWeighing(t)} disabled={busy||!ticketNumber.trim()||!gross.trim()||!tare.trim()||Number(gross)<=Number(tare)||!!printedNetInput.trim()&&Math.abs(Number(gross)-Number(tare)-Number(printedNetInput))>2}/><Button label="Volver a fotografiar" onPress={()=>void startTicket(t)} disabled={busy||ticketReadingBusy}/></>}
     </>}
  ...
  {profile?.role==='ADMIN'&&lots.some(l=>!trips.some(t=>t.harvest_order_id===l.harvest_order_id)&&!tripLots.some(x=>x.agave_lot_id===l.id))&&<><Text style={styles.heading}>Viajes históricos sin reserva</Text><Text>Lote</Text>{lots.filter(l=>!trips.some(t=>t.harvest_order_id===l.harvest_order_id)&&!tripLots.some(x=>x.agave_lot_id===l.id)).map(l=><Button key={l.id} label={`${tripLotId===l.id?'✓ ':''}${l.trace_code}`} onPress={()=>setTripLotId(l.id)}/>)}<Text>Chofer</Text>{drivers.map(d=><Button key={d.id} label={`${tripDriverId===d.id?'✓ ':''}${d.full_name}`} onPress={()=>setTripDriverId(d.id)}/>)}<Input label="Comprador y destilería de destino" value={destination} onChange={setDestination}/><Input label="Placa del camión" value={vehiclePlate} onChange={setVehiclePlate}/><Button label="Crear viaje histórico" onPress={createTrip} disabled={busy}/></>}
 </>}
