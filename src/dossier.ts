...
import {variance,DEFAULT_VARIANCE_KG,DEFAULT_VARIANCE_PERCENT} from './operations';
...
export function buildDossier(data:DossierData){
 ...
 const totalHarvest=data.links.reduce((a,l)=>a+Number(l.loaded_weight_kg||0),0);
 ...
 const trips=data.trips.map(t=>{
  ...
  const loaded=data.links.filter(l=>l.trip_id===t.id).reduce((a,l)=>a+Number(l.loaded_weight_kg||0),0);
  const accepted=ds.every(d=>d.accepted_weight_kg!=null)?ds.reduce((a,d)=>a+Number(d.accepted_weight_kg),0):null;
  const rejected=ds.every(d=>d.rejected_weight_kg!=null)?ds.reduce((a,d)=>a+Number(d.rejected_weight_kg),0):null;
  const v=variance(loaded||null,origin?.net_weight_kg??null,destination?.net_weight_kg??null,accepted,rejected,data.varianceLimitKg,data.varianceLimitPercent);
  return `<section class="trip"><h2>Camión / viaje ${esc(t.trace_code)}</h2><table>${row('Estado',t.status)}${row('Destino',t.destination_name)}${row('Chofer',driver?.full_name??'Sin identificar')}${row('Placa',t.vehicle_plate)}${row('Salida',date(t.departed_at))}${row('Llegada',date(t.arrived_at))}${row('Lotes vinculados',ls.map(l=>l.trace_code).join(', ')||'Sin lote')}${row('Peso de lotes cargados',kg(loaded))}</table>
  <h3>Pesajes y conciliación</h3><table><thead><tr><th>Punto</th><th>Bruto</th><th>Tara</th><th>Neto</th><th>Folio</th></tr></thead><tbody>${ws.map(w=>`<tr><td>${esc(w.weighing_type)}</td><td>${esc(kg(w.gross_weight_kg))}</td><td>${esc(kg(w.tare_weight_kg))}</td><td>${esc(kg(w.net_weight_kg))}</td><td>${esc(w.ticket_number)}</td></tr>`).join('')||'<tr><td colspan="5">Sin pesajes</td></tr>'}</tbody></table><table>${row('Campo − origen',kg(v.originDifference))}${row('Origen − destino',kg(v.transitDifference))}${row('Destino − aceptado − rechazado',kg(v.receiptDifference))}${row('Tolerancia aplicable',kg(v.threshold))}${row('Revisión',v.exceeded?'Supera tolerancia':'Dentro de tolerancia o pendiente de pesajes')}${row('Explicación registrada',data.varianceNotes?.find(n=>n.trip_id===t.id)?.reason)}</table>
 ...
 <h3>Lotes cosechados vinculados a ${esc(buyer)}</h3><table><thead><tr><th>Lote</th><th>Agaves</th><th>°Brix</th><th>Peso cargado</th><th>Viaje(s)</th></tr></thead><tbody>${data.lots.map(l=>`<tr><td>${esc(l.trace_code)}</td><td>${esc(l.agave_count)}</td><td>${esc(l.average_brix)}</td><td>${esc(kg(data.links.filter(x=>x.agave_lot_id===l.id&&relevantTrips.has(x.trip_id)).reduce((a,x)=>a+Number(x.loaded_weight_kg||0),0)))}</td><td>${esc(data.links.filter(x=>x.agave_lot_id===l.id&&relevantTrips.has(x.trip_id)).map(x=>data.trips.find(t=>t.id===x.trip_id)?.trace_code).filter(Boolean).join(', ')||'Pendiente')}</td></tr>`).join('')}</tbody></table>
 <div class="photos">${data.images.filter(i=>i.kind==='harvest').map(photo).join('')}</div>${trips}<h2>Resumen de conciliación</h2><table>${row('Lotes vinculados',data.lots.length)}${row('Camiones / viajes',data.trips.length)}${row('Peso de lotes cargados para este comprador',kg(totalHarvest))}${row('Peso aceptado en entregas',kg(totalAccepted))}${row('Peso rechazado',kg(totalRejected))}${row('Diferencia carga − aceptado − rechazado',kg(totalHarvest-totalAccepted-totalRejected))}${row('Tolerancia configurada',`${data.varianceLimitKg??DEFAULT_VARIANCE_KG} kg o ${data.varianceLimitPercent??DEFAULT_VARIANCE_PERCENT}% del origen, lo que sea mayor`)}</table><footer>Generado: ${esc(date(data.generatedAt))} · Identificador de jima: ${esc(data.harvest.id)} · Documento de control interno. Verifique los datos y anexos antes de compartirlo con ${esc(buyer)}. Las imágenes están incrustadas en este archivo y contienen información privada de la operación.</footer></main></body></html>`;
