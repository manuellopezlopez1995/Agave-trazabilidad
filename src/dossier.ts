...
export function buildDossier(data:DossierData){
 ...
 const trips=data.trips.map(t=>{
  ...
  <h3>Entrega acreditada con ticket de destino</h3>${ds.map(d=>`<table>${row('Entrega',d.trace_code)}${row('Comprador / destino',d.recipient_company)}${row('Estado',d.status)}${row('Fecha de cierre',date(d.received_at))}${row('Neto del ticket DESTINATION',kg(ws.find(w=>w.weighing_type==='DESTINATION')?.net_weight_kg))}${d.accepted_weight_kg!=null?row('Aceptado (registro histórico)',kg(d.accepted_weight_kg)):''}${d.rejected_weight_kg!=null?row('Rechazado (registro histórico)',kg(d.rejected_weight_kg)):''}</table>`).join('')||'<p>No se registró entrega.</p>'}
  ...
  <h3>Tickets de báscula y evidencias históricas</h3><div class="photos">${data.images.filter(i=>(i.kind==='ticket'&&ws.some(w=>w.id===i.label))||(i.kind==='delivery'&&ds.some(d=>i.label===`Entrega ${d.trace_code}`))).map(photo).join('')||'<p>Sin fotografías vinculadas.</p>'}</div></section>`;
 }).join('');
 ...
 <div class="photos">${data.images.filter(i=>i.kind==='harvest').map(photo).join('')}</div>${trips}<h2>Resumen de trazabilidad</h2><table>${row('Lotes vinculados',data.lots.length)}${row('Camiones / viajes',data.trips.length)}${row('Agaves jimados en lotes vinculados',totals.agaves)}${row('Peso total entregado según destino',kg(totals.delivered))}${row('Promedio de kilos por agave (entregado ÷ agaves jimados)',totals.kgPerAgave==null?'—':`${totals.kgPerAgave.toLocaleString('es-MX',{maximumFractionDigits:2})} kg/agave`)}${totals.accepted!=null?row('Peso aceptado (registros históricos)',kg(totals.accepted)):''}${totals.rejected!=null?row('Peso rechazado (registros históricos)',kg(totals.rejected)):''}</table><p>El promedio usa los agaves de los lotes vinculados. Si un lote se reparte entre compradores o viajes, su conteo no representa agaves asignados exclusivamente a este expediente. El ticket de destino acredita la entrega, no una aceptación comercial adicional.</p><footer>Generado: ${esc(date(data.generatedAt))} · Identificador de jima: ${esc(data.harvest.id)} · Documento de control interno. Verifique los datos y anexos antes de compartirlo con ${esc(buyer)}. Las imágenes están incrustadas en este archivo y contienen información privada de la operación.</footer></main></body></html>`;
