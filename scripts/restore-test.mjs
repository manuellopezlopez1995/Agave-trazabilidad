...
for(const bucket of ['harvest-evidence','weighing-tickets','delivery-evidence']){
 const response=await fetch(`${target}/storage/v1/bucket/${bucket}`,{headers:{Authorization:`Bearer ${key}`,apikey:key}});
 if(response.status===404){const created=await fetch(`${target}/storage/v1/bucket`,{method:'POST',headers:{Authorization:`Bearer ${key}`,apikey:key,'Content-Type':'application/json'},body:JSON.stringify({id:bucket,name:bucket,public:false})});if(!created.ok)throw Error(`No se creó el bucket privado ${bucket}`)}
 else if(!response.ok)throw Error(`No se verificó el bucket ${bucket}`);
}
